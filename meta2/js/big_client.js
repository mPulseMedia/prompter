...
(function() {
    ...
    function init() {
        ...
        if (toggle_data_create) {
            ...
            toggle_data_create.addEventListener('change', function() {
                ...
                localStorage.setItem('big_show_data_create', new_state);
                update_display();
            });
        }
        ...
        if (toggle_data_ref) {
            ...
            toggle_data_ref.addEventListener('change', function() {
                ...
                localStorage.setItem('big_show_data_ref', new_state);
                update_display();
            });
        }
    ...
    function function_show_update(show) {
        ...
        function_lines.forEach(func => {
            ...
            if (show) {
                ...
                // Mode 3: Data-only mode - show functions that have data operations
            ...
            if (should_show && !func.classList.contains('dir_hidden')) {
    ...
    function data_create_show_update(show) {
        ...
        // Re-evaluate function visibility based on data filters
    ...
    function update_display() {
        ...
        // Apply directory mode first (affects layout)
        dir_show_update(show_dir);
    ...
    function dir_show_update(show) {
        ...
        if (show) {
            // Directory mode - show full hierarchy with files and folders
            all_divs.forEach(div => {
                const hasFolder = div.querySelector('.big_folder_name');
                const hasFile = div.querySelector('.big_file_name');
                const isFunction = div.classList.contains('function_line');
                const isMethod = div.classList.contains('method_calls');
                const isDataCreate = div.classList.contains('data_create_line');
                const isDataRef = div.classList.contains('data_ref_line');
                
                // Reset all margins to hierarchical view
                const indent = parseInt(div.getAttribute('data-indent') || '0');
                div.style.marginLeft = (indent * 60) + 'px';
                
                if (hasFolder || hasFile) {
                    // Show folders and files in directory mode
                    div.style.display = 'flex';
                    div.classList.remove('dir_hidden');
                } else if (isFunction || isMethod || isDataCreate || isDataRef) {
                    // Keep functions/data visible but nested
                    div.classList.remove('dir_hidden');
                }
            });
            
            // Update file visibility based on content
            file_visibility_update();
        } else {
            // Flat mode - hide all folders and files
            all_divs.forEach(div => {
                const hasFolder = div.querySelector('.big_folder_name');
                const hasFile = div.querySelector('.big_file_name');
                const isFunction = div.classList.contains('function_line');
                ...
                if (hasFolder || hasFile) {
                    // Hide folders and files in flat mode
                    div.style.display = 'none';
                    div.classList.add('dir_hidden');
                } else if (isFunction || isDataCreate || isDataRef) {
                    // Show functions/data in flat list with minimal indent
                    div.style.marginLeft = '20px';
                    div.classList.remove('dir_hidden');
                    
                    // Also flatten method calls if visible
                    if (isMethod) {
